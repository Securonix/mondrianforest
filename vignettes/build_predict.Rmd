...
Each parameter $\textsf{f_scale[i]}$ should be interpreted as multiplying each dummy feature column generated from categorical column $i$ by $\textsf{f_scale[i]}$. Thus, argument $\textsf{f_scale=1/categorical_dummy_columns}$ gives each categorical column equal prior preference for each split.

Since columns $\textsf{X4_a}$ and $\textsf{X4_b}$ are now separate, taking $\textsf{f_scale=1/categorical_dummy_columns}$ doubles the prior split preference of those levels in aggregate relative to applying the same method to the unsplit $\textsf{X4}$, but gives each level in $\textsf{X4_a}$ slightly more preference because $1/7 > 1/9$. I use a higher $\textsf{lambda}$ here, which relates, along with the range of the variables, to the depth of the tree. Because the scales are lowered on average, I raise this parameter so the trees have approximately equal capacity.
...
Comparing the object sizes of the $\textsf{mf_extended}$, trained on all 500 observations of the unmodified data, to $\textsf{mf_scaled}$, trained on 500 observations of scaled data, suggests that $\textsf{mf_extended}$ is a more complex model. Nevertheless, scaling the variables results in improved performance likely because it limits excessive splitting on variable $\textsf{X3}$ which has a 6 order of magnitude range.
```{r, cache = TRUE}
